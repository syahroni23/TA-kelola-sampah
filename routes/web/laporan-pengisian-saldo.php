...
function exportData($conn) {
	...
	if($form['jenis'] == "excel") {
		...
		$spreadsheet = $reader->load("../format_excel/export/laporan-pengisian-saldo.xls");
		...
		if($form['kategori_filter'] == "Pilihan Otomatis") {
			...
			if($form['pilihan'] == "Hari Ini") {
				...
				$query = mysqli_query($conn, "SELECT t_pengisian_saldo.*, m_pelanggan.nama AS nama_pelanggan, m_petugas.nama AS nama_petugas FROM t_pengisian_saldo LEFT JOIN m_pelanggan ON t_pengisian_saldo.m_pelanggan_id = m_pelanggan.id LEFT JOIN m_petugas ON t_pengisian_saldo.m_petugas_id = m_petugas.id WHERE t_pengisian_saldo.is_deleted = '0' AND t_pengisian_saldo.created_at LIKE '$tanggal%' ORDER BY t_pengisian_saldo.kode ASC");
				...
				$query = mysqli_query($conn, "SELECT t_pengisian_saldo.*, m_pelanggan.nama AS nama_pelanggan, m_petugas.nama AS nama_petugas FROM t_pengisian_saldo LEFT JOIN m_pelanggan ON t_pengisian_saldo.m_pelanggan_id = m_pelanggan.id LEFT JOIN m_petugas ON t_pengisian_saldo.m_petugas_id = m_petugas.id WHERE t_pengisian_saldo.is_deleted = '0' AND t_pengisian_saldo.created_at BETWEEN '$tanggal_awal' AND '$tanggal_akhir' ORDER BY t_pengisian_saldo.kode ASC");
				...
				$query = mysqli_query($conn, "SELECT t_pengisian_saldo.*, m_pelanggan.nama AS nama_pelanggan, m_petugas.nama AS nama_petugas FROM t_pengisian_saldo LEFT JOIN m_pelanggan ON t_pengisian_saldo.m_pelanggan_id = m_pelanggan.id LEFT JOIN m_petugas ON t_pengisian_saldo.m_petugas_id = m_petugas.id WHERE t_pengisian_saldo.is_deleted = '0' ORDER BY t_pengisian_saldo.kode ASC");
			}else {
				$tanggal = date('Y');
				$periode_laporan = "Periode : " . formatDateIndonesia(date('Y'));
		...
		while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
			...
			$sheet->setCellValue('E'.$index, formatDateIndonesia(date('d F Y, H:i', strtotime($row['batas_waktu']))));
		...
		->setTitle("Laporan Pengisian Saldo")
		->setCompany("Kelola Sampah V2")
		->setSubject("Data Laporan");

		$spreadsheet->getProperties()->setDescription('Data Laporan Pengisian Saldo Periode ' . $periode_laporan);

		$spreadsheet->getProperties()->setKeywords("excel, laporan pengisian saldo")
		->setCategory("Master");

		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="laporan_pengisian_saldo_'.date('Ymd').'.xls"');
		...
		$pdf->SetTitle('Laporan Pengisian Saldo');
		$pdf->SetSubject('Data Laporan Pengisian Saldo');
		$pdf->SetKeywords('pdf, laporan pengisian saldo, pengisian saldo');
		...
		<td style="width: 54%;" align="left">Nama</td>
		...
		$pdf->Output('laporan_pengisian_saldo_'.date('Ymd').'.pdf', 'I');
	}
}
?>