...
function getData($conn) {
	...
	$columns = array(
		...
		1	=>	'm_hak_akses.kode',
		2	=>	'm_hak_akses.nama',
		3	=>	'm_hak_akses.status_akses',
		4	=>	'm_hak_akses.modified_at'
	);

	if(isset($_POST['isDeleted'])) {
		$sql .= " WHERE m_hak_akses.is_deleted = ".$_POST['isDeleted'];
	}

	if(isset($_POST['isStatusAkses']) && $_POST['isStatusAkses'] != "Semua") {
		$sql .= " AND m_hak_akses.status_akses = '".$_POST['isStatusAkses']."'";
	}

	if(isset($_POST['search']['value'])) {
		$search_value = $_POST['search']['value'];
		$sql .= " AND (m_hak_akses.kode LIKE '%".$search_value."%'";
		$sql .= " OR m_hak_akses.status_akses LIKE '%".$search_value."%'";
		$sql .= " OR m_hak_akses.nama LIKE '%".$search_value."%')";
	}
	...
	if(isset($_POST['order'])) {
		...
		$sql .= " ORDER BY m_hak_akses.kode ASC";
	}
	...
	while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
		if($row['is_deleted'] == 0) {
			...
			<a href='edit/hak-akses/".$row['id']."' class='btn btn-primary btn-sm btn-action mr-1 cursor-pointer'><i class='fas fa-edit fa-sm'></i></a>
			</div>
		...
		if($row['status_akses'] == "Dapat Akses") {
			$badge = "<span class='badge badge-success'>".$row['status_akses']."</span>";
		}else {
			$badge = "<span class='badge badge-danger'>".$row['status_akses']."</span>";
		}
		$sub_array = array();
		$sub_array[] = null;
		$sub_array[] = "<a href='view/hak-akses/".$row['id']."' class='link'>".$row['kode']."</a>";
		$sub_array[] = $row['nama'];
		$sub_array[] = $badge;
...
function addData($conn) {
	$form = $_POST;
	$form['kode'] = getCodeIncrement($conn, 'm_hak_akses', 'kode', "KLS/HA/", 5);
	$form['nama'] = mysqli_escape_string($conn, $form['nama']);

	$arrHakAkses = [];
	if(issetEmpty($form['akses'])) {
		foreach ($form['akses'] as $value) {
			$arrHakAkses[$value] = true;
		}
	}
	$form['akses'] = mysqli_escape_string($conn, json_encode($arrHakAkses));

	try {
		$models = toInsertData($conn, 'm_hak_akses', $form, true);
...
function updateData($conn) {
	$getData = toGetDataDetail($conn, 'm_hak_akses', $_POST['id']);
	$form = $_POST;
	$form['nama'] = mysqli_escape_string($conn, $form['nama']);
	...
	try {
		$models = toUpdateData($conn, 'm_hak_akses', $form, $form['id'], null, true);
		if($models['status'] == true) {
			echo json_encode([
				'status' => 1,
				'status_code' => 200,
...
function trashData($conn) {
	try {
		...
		$models = toTrashData($conn, 'm_hak_akses', $formPost['id'], null, true);
...
function deleteData($conn) {
	try {
		...
		$checkData = checkDataExist($conn, 'm_hak_akses', 'm_hak_akses_id', $formPost['id']);
		...
		$getData = toGetDataDetail($conn, 'm_hak_akses', $formPost['id']);

		$models = toDeleteData($conn, 'm_hak_akses', $formPost['id']);
...
function restoreData($conn) {
	try {
		...
		$models = toRestoreData($conn, 'm_hak_akses', $formPost['id'], null, true);
