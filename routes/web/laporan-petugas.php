...
function exportData($conn) {
	...
	if($form['jenis'] == "excel") {
		...
		$spreadsheet = $reader->load("../format_excel/export/laporan-petugas.xls");
		...
		if($form['kategori_filter'] == "Pilihan Otomatis") {
			...
			if($form['pilihan'] == "Hari Ini") {
				...
				$query = mysqli_query($conn, "SELECT * FROM m_petugas WHERE is_deleted = '0' AND created_at LIKE '$tanggal%' ORDER BY kode ASC");
				...
				$query = mysqli_query($conn, "SELECT * FROM m_petugas WHERE is_deleted = '0' AND created_at BETWEEN '$tanggal_awal' AND '$tanggal_akhir' ORDER BY kode ASC");
				...
				$query = mysqli_query($conn, "SELECT * FROM m_petugas WHERE is_deleted = '0' ORDER BY kode ASC");
			}else {
				$tanggal = date('Y');
				$periode_laporan = "Periode : " . formatDateIndonesia(date('Y'));
		...
		->setTitle("Laporan Petugas")
		->setCompany("Kelola Sampah V2")
		->setSubject("Data Laporan");

		$spreadsheet->getProperties()->setDescription('Data Laporan Petugas Periode ' . $periode_laporan);

		$spreadsheet->getProperties()->setKeywords("excel, laporan petugas")
		->setCategory("Master");

		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="laporan_petugas_'.date('Ymd').'.xls"');
		...
		$pdf->SetTitle('Laporan Petugas');
		$pdf->SetSubject('Data Laporan Petugas');
		$pdf->SetKeywords('pdf, laporan petugas, petugas');
		...
		$pdf->Output('laporan_petugas_'.date('Ymd').'.pdf', 'D');
	}
}
?>