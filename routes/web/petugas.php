...
function getData($conn) {
	...
	$columns = array(
		...
		4	=>	'm_petugas.saldo',
		5	=>	'm_petugas.alamat',
		6	=>	'm_petugas.modified_at'
	);

	if(isset($_POST['isDeleted'])) {
		$sql .= " WHERE m_petugas.is_deleted = ".$_POST['isDeleted'];
	}

	if(isset($_POST['search']['value'])) {
		$search_value = $_POST['search']['value'];
		$sql .= " AND (m_petugas.kode LIKE '%".$search_value."%'";
		$sql .= " OR m_petugas.nama LIKE '%".$search_value."%'";
		$sql .= " OR m_petugas.telepon LIKE '%".$search_value."%'";
		$sql .= " OR m_petugas.saldo LIKE '%".$search_value."%'";
		$sql .= " OR m_petugas.alamat LIKE '%".$search_value."%')";
	}
	...
	if(isset($_POST['order'])) {
		...
		$sql .= " ORDER BY m_petugas.kode ASC";
	}
	...
	while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
		if($row['is_deleted'] == 0) {
			...
			<a href='edit/petugas/".$row['id']."' class='btn btn-primary btn-sm btn-action mr-1 cursor-pointer'><i class='fas fa-edit fa-sm'></i></a>
			</div>
		...
		$sub_array = array();
		$sub_array[] = null;
		$sub_array[] = "<a href='view/petugas/".$row['id']."' class='link'>".$row['kode']."</a>";
		$sub_array[] = $row['nama'];
...
function addData($conn) {
	$form = $_POST;
	$form['kode'] = getCodeIncrement($conn, 'm_petugas', 'kode', "KLS/PT/", 5);
	...
	if($countTeleponPetugas > 0) {
		echo json_encode([
			'status' => 0,
			'status_code' => 400,
			'message' => "Telepon sudah digunakan oleh petugas lain",
			"info_error" => null,
			'data' => null
		], JSON_PRETTY_PRINT);
		breakResponse();
	...
	if($countEmailPetugas > 0) {
		echo json_encode([
			'status' => 0,
			'status_code' => 400,
			'message' => "E-mail sudah digunakan oleh petugas lain",
			"info_error" => null,
			'data' => null
		], JSON_PRETTY_PRINT);
		breakResponse();
	...
	try {
		$models = toInsertData($conn, 'm_petugas', $form, true);
...
function updateData($conn) {
	$getData = toGetDataDetail($conn, 'm_petugas', $_POST['id']);
	$form = $_POST;
	$form['nama'] = mysqli_escape_string($conn, $form['nama']);
	$form['tempat_lahir'] = mysqli_escape_string($conn, $form['tempat_lahir']);
	$form['tanggal_lahir'] = date('Y-m-d', strtotime($form['tanggal_lahir']));
	...
	try {
		$models = toUpdateData($conn, 'm_petugas', $form, $form['id'], null, true);
		if($models['status'] == true) {
			if(issetEmpty($_FILES['avatar']['name'])) {
...
function trashData($conn) {
	try {
		...
		$models = toTrashData($conn, 'm_petugas', $formPost['id'], null, true);
...
function deleteData($conn) {
	try {
		...
		$checkData = checkDataExist($conn, 'm_petugas', 'm_petugas_id', $formPost['id']);
		...
		$getData = toGetDataDetail($conn, 'm_petugas', $formPost['id']);

		if(issetEmpty($getData['avatar']) && file_exists("../../assets/img/avatar/".$getData['avatar']) && $getData['avatar'] != "default-avatar.png") {
			unlinkFile("../../assets/img/avatar/".$getData['avatar']);
		}

		$models = toDeleteData($conn, 'm_petugas', $formPost['id']);
...
function restoreData($conn) {
	try {
		...
		$models = toRestoreData($conn, 'm_petugas', $formPost['id'], null, true);
