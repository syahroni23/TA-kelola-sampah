...
function exportData($conn) {
	...
	if($form['jenis'] == "excel") {
		...
		$spreadsheet = $reader->load("../format_excel/export/laporan-pengambilan-saldo.xls");
		...
		if($form['kategori_filter'] == "Pilihan Otomatis") {
			...
			if($form['pilihan'] == "Hari Ini") {
				...
				$query = mysqli_query($conn, "SELECT t_pengambilan_saldo.*, m_bank.nama AS nama_bank, m_pelanggan.nama AS nama_pelanggan, m_petugas.nama AS nama_petugas FROM t_pengambilan_saldo LEFT JOIN m_bank ON t_pengambilan_saldo.m_bank_id = m_bank.id LEFT JOIN m_pelanggan ON t_pengambilan_saldo.m_pelanggan_id = m_pelanggan.id LEFT JOIN m_petugas ON t_pengambilan_saldo.m_petugas_id = m_petugas.id WHERE t_pengambilan_saldo.is_deleted = '0' AND t_pengambilan_saldo.created_at LIKE '$tanggal%' ORDER BY t_pengambilan_saldo.kode ASC");
				...
				$query = mysqli_query($conn, "SELECT t_pengambilan_saldo.*, m_bank.nama AS nama_bank, m_pelanggan.nama AS nama_pelanggan, m_petugas.nama AS nama_petugas FROM t_pengambilan_saldo LEFT JOIN m_bank ON t_pengambilan_saldo.m_bank_id = m_bank.id LEFT JOIN m_pelanggan ON t_pengambilan_saldo.m_pelanggan_id = m_pelanggan.id LEFT JOIN m_petugas ON t_pengambilan_saldo.m_petugas_id = m_petugas.id WHERE t_pengambilan_saldo.is_deleted = '0' AND t_pengambilan_saldo.created_at BETWEEN '$tanggal_awal' AND '$tanggal_akhir' ORDER BY t_pengambilan_saldo.kode ASC");
				...
				$query = mysqli_query($conn, "SELECT t_pengambilan_saldo.*, m_bank.nama AS nama_bank, m_pelanggan.nama AS nama_pelanggan, m_petugas.nama AS nama_petugas FROM t_pengambilan_saldo LEFT JOIN m_bank ON t_pengambilan_saldo.m_bank_id = m_bank.id LEFT JOIN m_pelanggan ON t_pengambilan_saldo.m_pelanggan_id = m_pelanggan.id LEFT JOIN m_petugas ON t_pengambilan_saldo.m_petugas_id = m_petugas.id WHERE t_pengambilan_saldo.is_deleted = '0' ORDER BY t_pengambilan_saldo.kode ASC");
			}else {
				$tanggal = date('Y');
				$periode_laporan = "Periode : " . formatDateIndonesia(date('Y'));
		...
		while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
			...
			$nama = isset($row['nama_pelanggan']) && !empty($row['nama_pelanggan']) ? $row['nama_pelanggan'] : $row['nama_petugas'];
            ...
            $sheet->setCellValue('D'.$index, $row['nominal']);
            $sheet->setCellValue('E'.$index, $row['nama_bank']);
			$sheet->setCellValue('F'.$index, $row['nomor_rekening']);
			$sheet->setCellValue('G'.$index, $row['atas_nama']);
            $sheet->setCellValue('H'.$index, $row['status']);
			$sheet->setCellValue('I'.$index, $status);
			$sheet->setCellValue('J'.$index, formatDateIndonesia(date('d F Y, H:i', strtotime($row['created_at']))));
		...
		->setTitle("Laporan Pengambilan Saldo")
		->setCompany("Kelola Sampah V2")
		->setSubject("Data Laporan");

		$spreadsheet->getProperties()->setDescription('Data Laporan Pengambilan Saldo Periode ' . $periode_laporan);

		$spreadsheet->getProperties()->setKeywords("excel, laporan pengambilan saldo")
		->setCategory("Master");

		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="laporan_pengambilan_saldo_'.date('Ymd').'.xls"');
		...
		$pdf->SetTitle('Laporan Pengambilan Saldo');
		$pdf->SetSubject('Data Laporan Pengambilan Saldo');
		$pdf->SetKeywords('pdf, laporan pengambilan saldo, pengambilan saldo');
		...
		<td style="width: 28%;" align="left">Nama</td>
		<td style="width: 26%;" align="left">Bank</td>
        <td style="width: 15%;" align="right">Nominal</td>
		</tr>
		</thead>
		<tbody>';
		...
		while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
            ...
            <td style="width: 28%;" align="left">'.$nama.'</td>
            <td style="width: 26%;" align="left">'.$row['nama_bank'].'</td>
			<td style="width: 15%;" align="right">Rp. '.number_format($row['nominal']).'</td>
			</tr>';
		}
		...
		$pdf->Output('laporan_pengambilan_saldo_'.date('Ymd').'.pdf', 'D');
	}
}
?>