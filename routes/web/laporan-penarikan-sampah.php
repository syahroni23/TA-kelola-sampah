...
function exportData($conn) {
	...
	if($form['jenis'] == "excel") {
		...
		$spreadsheet = $reader->load("../format_excel/export/laporan-penarikan-sampah.xls");
		...
		if($form['kategori_filter'] == "Pilihan Otomatis") {
			...
			if($form['pilihan'] == "Hari Ini") {
				...
				$query = mysqli_query($conn, "SELECT t_penarikan_sampah.*, m_pelanggan.nama AS nama_pelanggan FROM t_penarikan_sampah LEFT JOIN m_pelanggan ON t_penarikan_sampah.m_pelanggan_id = m_pelanggan.id WHERE t_penarikan_sampah.is_deleted = '0' AND t_penarikan_sampah.created_at LIKE '$tanggal%' ORDER BY t_penarikan_sampah.kode ASC");
				...
				$query = mysqli_query($conn, "SELECT t_penarikan_sampah.*, m_pelanggan.nama AS nama_pelanggan FROM t_penarikan_sampah LEFT JOIN m_pelanggan ON t_penarikan_sampah.m_pelanggan_id = m_pelanggan.id WHERE t_penarikan_sampah.is_deleted = '0' AND t_penarikan_sampah.created_at BETWEEN '$tanggal_awal' AND '$tanggal_akhir' ORDER BY t_penarikan_sampah.kode ASC");
				...
				$query = mysqli_query($conn, "SELECT t_penarikan_sampah.*, m_pelanggan.nama AS nama_pelanggan FROM t_penarikan_sampah LEFT JOIN m_pelanggan ON t_penarikan_sampah.m_pelanggan_id = m_pelanggan.id WHERE t_penarikan_sampah.is_deleted = '0' ORDER BY t_penarikan_sampah.kode ASC");
			}else {
				$tanggal = date('Y');
				$periode_laporan = "Periode : " . formatDateIndonesia(date('Y'));
		...
		while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
			...
			if($row['status'] == "Pending") {
				$status_transaksi = "Minta Diambil";
			}else if($row['status'] == "Proses") {
				$status_transaksi = "Proses Ambil";
			}else if($row['status'] == "Selesai") {
				$status_transaksi = "Sudah Diambil";
			}else {
				$status_transaksi = "Minta Diambil";
			}
            ...
            $sheet->setCellValue('D'.$index, $status_transaksi);
			$sheet->setCellValue('E'.$index, $status);
			$sheet->setCellValue('F'.$index, formatDateIndonesia(date('d F Y, H:i', strtotime($row['created_at']))));
		...
		->setTitle("Laporan Penarikan Sampah")
		->setCompany("Kelola Sampah V2")
		->setSubject("Data Laporan");

		$spreadsheet->getProperties()->setDescription('Data Laporan Penarikan Sampah Periode ' . $periode_laporan);

		$spreadsheet->getProperties()->setKeywords("excel, laporan penarikan sampah")
		->setCategory("Master");

		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="laporan_penarikan_sampah_'.date('Ymd').'.xls"');
		...
		$pdf->SetTitle('Laporan Penarikan Sampah');
		$pdf->SetSubject('Data Laporan Penarikan Sampah');
		$pdf->SetKeywords('pdf, laporan penarikan sampah, penarikan sampah');
		...
		<td style="width: 54%;" align="left">Pelanggan</td>
		...
		while($row = allRowValidateHTML(mysqli_fetch_array($query, MYSQLI_ASSOC))) {
            ...
            <td style="width: 54%;" align="left">'.$row['nama_pelanggan'].'</td>
            <td style="width: 15%;" align="center">'.$row['status'].'</td>
			</tr>';
		}
		...
		$pdf->Output('laporan_penarikan_sampah_'.date('Ymd').'.pdf', 'D');
	}
}
?>