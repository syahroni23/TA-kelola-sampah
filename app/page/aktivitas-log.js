...
function actionDataTables(table) {
	$(document).on('click', '.trashed', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				trashData("../routes/web/aktivitas-log.php", id, table);
			}
		});
	});
	...
	$(document).on('click', '.restored', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				restoreData("../routes/web/aktivitas-log.php", id, table);
			}
		});
	});
	...
	$(document).on('click', '.deleted', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				deleteData("../routes/web/aktivitas-log.php", id, table);
			}
		});
	});
}
...
var table = $('#datatables-serverside').DataTable({
	...
	"order": [],
	"ajax": {
		"url": "../routes/web/aktivitas-log.php",
		"type": "post",
		"data": function (d) {
			return $.extend({}, d, {
	...
	"aoColumns": [{"sClass": "text-center"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-center"}],
	"oLanguage": {
