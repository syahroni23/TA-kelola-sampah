function actionDataTables(table) {
	$(document).on('click', '.trashed', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				trashData("../routes/web/pengguna.php", id, table);
			}
		});
	});
	...
	$(document).on('click', '.restored', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				restoreData("../routes/web/pengguna.php", id, table);
			}
		});
	});
	...
	$(document).on('click', '.deleted', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				deleteData("../routes/web/pengguna.php", id, table);
			}
		});
	});
}
...
var table = $('#datatables-serverside').DataTable({
	...
	"aoColumns": [{"sClass": "text-center"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-center"}],
	"oLanguage": {
