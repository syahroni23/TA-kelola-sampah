function actionDataTables(table) {
	$(document).on('click', '.trashed', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				trashData("../routes/web/hak-akses.php", id, table);
			}
		});
	});
	...
	$(document).on('click', '.restored', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				restoreData("../routes/web/hak-akses.php", id, table);
			}
		});
	});
	...
	$(document).on('click', '.deleted', function(e) {
		...
		Swal.fire({
			...
			if(result.isConfirmed) {
				deleteData("../routes/web/hak-akses.php", id, table);
			}
		});
	});
}
...
var table = $('#datatables-serverside').DataTable({
	...
	"ajax": {
		...
		"data": function (d) {
			return $.extend({}, d, {
				...
				"isStatusAkses": $('#isStatusAkses').val(),
				"function": "getData"
			});
		}
	},
	...
	"aoColumns": [{"sClass": "text-center"}, {"sClass": "text-left"}, {"sClass": "text-left"}, {"sClass": "text-center"}, {"sClass": "text-left"}, {"sClass": "text-center"}],
	"oLanguage": {
...
$(document).ready(function() {
	...
	$('#isStatusAkses').on('change', function(e) {
		e.preventDefault();
		table.draw();
	});
});